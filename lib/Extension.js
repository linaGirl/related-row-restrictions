!function() {
    ...
    module.exports = new Class({
        ...
        , applyRestrictionsOnQuery: function(resource) {
            ...
            // apply restrictions targeted specifically on this entity
            
            if (!resource.dontApplyRowRestrictions) {
            ...
            } else log.error('resource.dontApplyRowRestrictions', resource.name);
        }
        ...
        /**
        * make sure a specif entity is not filtered using row restrictions
        */
        , ignoreRestrictions() {
            this.getresource().dontApplyRowRestrictions = true;
            return this;
        }
        ...
        /**
        * translate actions from sql actions into crud actions
        */
         ...
         *
         * @param {Resource} resource the resource form the queryBuidler
         * @param {Restriction[]} restrictions an array containing restriction instances
         * @param {Boolen} isGlobal indicates if the array contains global restrictions
         *
         * @throws {Error} if a restriction cannot be applied but must be applied or has an invalid
         *                 format
         */
        , applyRestrictions: function(resource, restrictions, isGlobal) {
            try {
                ...
                // we need to access the orm object
                ...
                if (debug) {
                    err.message = `[${resource.name}] ${err.message}`;
                    log('row-restriction variables');
                    log(resource.getRootResoure().rowRestrictionVariables);
                    log('restrictions');
                    log(restrictions);
                    log('query');
                    log(resource.query);
        ...
        , restrict: function(rowRestrictions) {
            ...
            // convert
            rowRestrictions = rowRestrictions.map((restriction) => {
                restriction = Object.assign({}, restriction);
        ...
        , applyQueryBuilderMethods: function(definition, classDefinition) {
            ...
            // allow the user to ignore restrictions on specific models
            classDefinition.ignoreRestrictions = this.ignoreRestrictions;
        }
