    ...
    module.exports = new Class({
        ...
        , onBeforeSave: function(model, transaction, callback) {
            ...
            if (model._rowRestrictions) {
                try {
                    this.applyModelRestrictions(model, model._rowRestrictions.get(model.getEntityName(), action));
                    this.applyModelRestrictions(model, model._rowRestrictions.getGlobal(action), true);
                } catch (e) {
                    return callback(e);
                }
                callback();
        ...
        , applyRestrictionsOnQuery: function(resource) {
            ...
            const restrictions = rootResource.rowRestrictions.filter(r => r.resources.includes(resource.name));
            ...
            restrictions.forEach(function(restriction) {
                ...
                if (!isGlobal || (isGlobal && targetQueryBuilder && targetQueryBuilder.hasColumn(restriction.property))) {
                    ...
                    switch (restriction.valueType) {
                        case 'variable':
                            if (!rootResource.rowRestrictionVariables[restriction.value.value]) throw new Error('The variable «'+restriction.value.value+'» was not set on the queryBuilder, cannot apply the row restriction!');
                            filter = ORM[restriction.comparator](rootResource.rowRestrictionVariables[restriction.value.value]);
                            break;


                        case 'function':
                            filter = ORM[restriction.comparator](thisContext.executeFunction(restriction.value.value, rootResource.rowRestrictionVariables));
                            break;


                        case 'constant':
                            filter = ORM[restriction.comparator](restriction.value.value);
                            break;
